...
* `metric`: The metric to use, defaults to `Euclidean`. This is one of the `Metric` types defined in the `Distances.jl` packages. It is possible to define your own metrics by creating new types that are subtypes of `Metric`. (see below)
...
(::MyMetric)(x,y) = Euclidean()(x,y)

# Now its useable!
BruteTree(randn(2, 100), MyMetric())
```
