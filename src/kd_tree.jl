...
function _inrange(tree::KDTree,
    ...
    return inrange_kernel!(tree, 1, point, eval_pow(tree.metric, radius), idx_in_ball,
            tree.hyper_rec, init_min, skip, false)
end
...
function inrange_kernel!(tree::KDTree,
    ...
    count += inrange_kernel!(tree, close, point, r, idx_in_ball, hyper_rec_close, min_dist, skip, unique)
    ...
    diff_tot = eval_diff(M, split_diff_pow, ddiff_pow, split_dim)
    new_min = eval_reduce(M, min_dist, diff_tot)
    count += inrange_kernel!(tree, far, point, r, idx_in_ball, hyper_rec_far, new_min, skip, unique)
    return count
end
