...
@testset "knn skip" begin
    ...
    @testset "skipself keyword" begin
        data = rand(3, 25)
        tree = KDTree(data)
        idxs_skip, dists_skip = knn(tree, data, 1, true; skipself=true)
        ...
        nn_idx, nn_dist = nn(tree, data; skipself=true)
        @test nn_idx == expected_idxs
        @test nn_dist ≈ expected_dists

        # skipself combines with a custom skip predicate
        block_idx = 5
        idxs_blocked, _ = knn(tree, data, 1, true, j -> j == block_idx; skipself=true)
